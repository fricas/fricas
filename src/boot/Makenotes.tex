...
\begin{abstract}
  \Tool{Axiom} is built in layers. The first layer is constructed into
...
\section{Boot To Common Lisp Translators}
...
functions.  It also contains the entry points to the Boot translator.
...
passed to the translator rather than being passed itself.
...
A [[PlainLine]] is delivered to the translator as is.
...
A [[Say]] outputs the remainder of the line to the console,
   delivering nothing to the translator.
...
An [[EvalStrings]] also translates and evaluates the rest of the line
   ...
   of strings which are then delivered to the translator
...
are delivered to the translator. In any case the lines after
...
There are two ways to quote an identifier: either 'name or "name", which
...
column will be called a \emph{pile}.  The translator first tokenizes the
...
the expression itself. If it is immediately preceded by a CONS
...
than their values, and whose result is formed by first substituting
...
if there, it returns it; if not, it computes the result and stores it in the
table.
