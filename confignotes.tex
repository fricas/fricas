...
\begin{abstract}
  ...
  \Tool{FriCAS} for build from source codes.
  ...
  \Tool{FriCAS} interpreter, compiler, libraries, and auxiliary tools
  where appropriate.
\end{abstract}
...
minimum environment for building \Tool{FriCAS}.  This effort
...
The task is compounded by the fact that the existing \Tool{FriCAS} system
is complex and very poorly documented, with blatantly conflicting or
questionable codes.

The \Tool{FriCAS} system is written for the most part in Lisp, or
...
is where \Tool{FriCAS} will run.  Note that in full generality, there is
...
fricas_builddir=`pwd`/build/$build
...
fricas_targetdir=`pwd`/target/$target
...
For the moment, the \Tool{FriCAS} base source code is written
...
platform.  But compilers do.  And \Tool{FriCAS} does because, among
...
The \Tool{FriCAS} system does not that support that level of
...
Although \Tool{FriCAS} does not support cross build yet, let
...
fricas_build_sharedir=$fricas_builddir/share
@

\section{Host characteristics}

As mentioned in the introduction, a small part of \Tool{FriCAS} is
...
not strictly necessary, that is the way \Tool{FriCAS} source code
...
Some parts of \Tool{FriCAS} manipulate files and directories.  They
...
sockets.  This is required for interfacing \Tool{FRICASsys}
and \Tool{Superman}.  Notice that ideally, we should decouple
that interface in such a way that we can still build \Tool{FriCAS}
...
<<gcl options>>=
fricas_host_has_libbfd=
AC_CHECK_HEADER([bfd.h])
AC_HAVE_LIBRARY([bfd], [fricas_host_has_libbfd=yes])

fricas_gcl_bfd_option=
if test x"$ac_cv_header_bfd_h" = xyes \
    && test x"$fricas_host_has_libbfd" = xyes; then
    fricas_gcl_bfd_option="--disable-dynsysbfd"
else
    fricas_gcl_bfd_option="--disable-statsysbfd --enable-locbfd"
fi
@

\Tool{GCL} has an elaborate memory management system and
\Tool{FriCAS} seems to
put ``unusual'' pressure on it.  Here we specify some values that have
been empirically known to work.
<<gcl options>>=
# fricas_gcl_mm_option="--enable-maxpage=256*1024"
@

Furthermore, we don't need (at the moment) \Tool{GCL} to build support for
X Window system or TCL/TK:
<<gcl options>>=
fricas_gcl_x_option="--disable-tkconfig --disable-x --disable-xgcl"
@

Other aspects depend on the platform being considered.


\Tool{FriCAS} source code had developed the appalling and irritating habit
of testing for
...
\Tool{FriCAS}'s dependencies on external tools:  \Tool{FriCAS} relies on
...
FriCAS sources is literate, and it uses the \Tool{noweb} technology.
...
There are many platforms on which \Tool{noweb} is inexistent.  FriCAS source
...
AC_ARG_WITH([included-noweb],
            ...
            [case $withval in
                yes) if test -f $fricas_top_srcdir/zips/noweb-2.10a.tgz ; then
...
if test x$fricas_build_noweb != xyes ; then
    ...
    if test -z $NOTANGLE -o -z $NOWEAVE ; then
	   ...
	   AC_MSG_ERROR([noweb utils are missing but --without-included-noweb is specified])
        elif test -f $fricas_top_srcdir/zips/noweb-2.10a.tgz ; then
           fricas_build_noweb=yes
        else
...
the option \verb!--enable-gcl! is specified then \Tool{FriCAS}
...
if test -z $fricas_fasl_type; then
    ...
    fricas_save_IFS=$IFS
    IFS=' '
    fricas_fasl_type=`$fricas_lisp $fricas_quiet_flags $fricas_eval_flags '(progn (format t "fricas_fasl_type=~a" (pathname-type (compile-file-pathname "foo.lisp"))) (quit))'`
...
We strive for making \Tool{FriCAS}'s build system integrate as seamlessly as
...
\begin{description}
\item \verb!--prefix!:
  By default, \Tool{FriCAS}'s build system will install files
...
\subsection{\Tool{FriCAS}-specific options}
\label{sec:config-options:fricas-specific}

\begin{description}
\item \verb!--enable-gcl!:
  \Tool{FriCAS} needs an implementation of Lisp to support its
  runtime system.  At the moment, GNU Common Lisp (\Tool{GCL} for short)
  is used.  This options instructs \Tool{FriCAS} to build its own copy
  of \Tool{GCL}.  Use \verb!--disable-gcl! to prevent FriCAS
...
appropriate for all C sub-parts of \Tool{FriCAS} to include.
...
fricas_src_subdirs="lib lisp boot interp share algebra input etc doc"
