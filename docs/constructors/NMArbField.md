...
> **Kind**: Domain &nbsp;|&nbsp; \[[Source](https://github.com/gvanuxem/jlfricas/blob/master/src/algebra/jnball.spad#L1)\] &nbsp;|&nbsp; **Group**: NM — Nemo (FLINT)

## Description

NMArbField implements fixed precision ball arithmetic using the Nemo Julia package - based on the Arb library.

**NMArbField(p: PositiveInteger) is a domain constructor**  
**Abbreviation for NMArbField is NARB**  
...
--- Operations ---
 ...
 catalan : () -> %                                      ceiling : % -> %
 characteristic : () -> NonNegativeInteger              coerce : Float -> %
 coerce : JLFloat64 -> %                                coerce : Fraction(Integer) -> %
 coerce : % -> %                                        coerce : Integer -> %
 coerce : % -> JLObject                                 coerce : % -> OutputForm
 commutator : (%, %) -> %                               contains? : (%, JLFloat) -> Boolean
 contains? : (%, NMFraction(NMInteger)) -> Boolean      contains? : (%, NMInteger) -> Boolean
 contains? : (%, %) -> Boolean                          containsNegative? : % -> Boolean
 ...
 digits : () -> PositiveInteger                         equal? : (%, %) -> Boolean
 ...
 jnrb : Integer -> %                                    jnrb : String -> %
 jnrb : NMExactCalciumField -> %                        jnrb : NMAlgebraicNumber -> %
 khinchin : () -> %                                     latex : % -> String
 lcm : (%, %) -> %                                      lcm : List(%) -> %
 ldexp : (%, NMInteger) -> %                            leftPower : (%, PositiveInteger) -> %
